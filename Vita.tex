...
\NewDocumentEnvironment{citemize}{o}{%
  \IfValueTF{#1}{%
    \begin{itemize}[nosep,leftmargin=*,#1]% chktex 15
  }{%
    \begin{itemize}[nosep,leftmargin=*]% chktex 15
  }%
}{%
  \end{itemize}%
}%

\NewDocumentEnvironment{vcitemize}{o}{%
  \IfValueTF{#1}{%
    \begin{itemize}[nosep,leftmargin=*,% chktex 15
...
\newadjustboxenv{minipagebox}[2][0]{%
  valign=m,%
  minipage={#2},%
  margin=0 0 0 {#1},%
}%
...
\NewDocumentEnvironment{TCBItems}{}{%
  \begin{tcolorbox}\begin{citemize}%
}{%
  \end{citemize}\end{tcolorbox}%
}%
...
\RequirePackage[style=numeric,backend=biber,datamodel=supervisor]{biblatex}%
...
\begin{minipage}[t]{0.575\textwidth}
  ...
  {%
    ...
    \item Part of the team handling Zalando SE’s \emph{core data}: articles and
      sales.
    \item
      \begingroup%
      \tcbset{%
      ...
      \end{minipagebox}%
      \hfill%
      \begin{minipagebox}[20pt]{.2\linewidth}
        ...
        \begin{tikzpicture}[overlay,remember picture]
          \node[
          draw,rectangle,thick,dotted,fit=(Spark),%
          inner sep=6pt,%
      ...
      \begin{minipagebox}[20pt]{.15\linewidth}
        ...
        \begin{tikzpicture}[%
          ...
          >=stealth,shorten >=2pt,shorten <=2pt%
          ]
          \draw[->] (Kafka) to (Spark);
          ...
          \draw[<->] (Presto) to (S3);
          \draw[->] (S3.south) to[out=-160,in=-20]%
          node[near start,below right=-2pt and -2pt,scale=0.75]%
          {SNS \& SQS} (Spark.south);
          \draw[->] (Spark.north east) to[out=20,in=160] (Exasol.north west);
          \node[draw,rectangle,thick,dotted,fit=(Exasol) (Presto),%
          inner sep=6pt,%
